...
public class LC02105_WateringPlantsIi {
    public static void main(String[] args) {
        Solution solution = new LC02105_WateringPlantsIi().new Solution();
    }
    ...
    class Solution {
        public int minimumRefill(int[] plants, int capacityA, int capacityB) {
            ...
            while (indexA < indexB) {
                if (restA >= plants[indexA]) {
                    restA -= plants[indexA++];
                } else {
                    ans++;
                    restA = capacityA - plants[indexA++];
                }
                if (restB >= plants[indexB]) {
                    restB -= plants[indexB--];
                } else {
                    ans++;
                    restB = capacityA - plants[indexB--];
                }
            }
            if (indexA == indexB) {
                if (restA < restB) {
